...
limit_existence = 'ativado: $%d' % price_limit if price_limit > 0.0 else 'desativado'
...
def express():
    with sync_playwright() as p:
        ...
        while True:
            start_time = time.time()
            try:
                assert page.inner_text('//*[@id="title-and-refresh-line"]/div/h5') != 'Waiting for new jobs...'
                # assert page.title() != "Rev - Find Work"
